<?php
$isEdit = isset($editHarga) && $editHarga;
$user = currentUser();
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title><?= $isEdit ? 'Edit' : 'Atur' ?> Harga - <?= APP_NAME ?></title>
    <style>
        * { box-sizing: border-box; } body { margin: 0; font-family: 'Segoe UI', sans-serif; background: #f5f7fa; }
        ...
        .info-box { background: #e3f2fd; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; color: #1565c0; font-size: 0.9rem; }
    </style>
</head>
<body>
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <h1><?= $isEdit ? '✏️ Edit' : '➕ Atur' ?> Harga Sampah</h1>
                <div class="breadcrumb"><a href="<?= base_url('pengelola/dashboard') ?>">Dashboard</a> / <a href="<?= base_url('pengelola/harga') ?>">Harga</a> / <?= $isEdit ? 'Edit' : 'Baru' ?></div>
            </div>

            <?php if (!$isEdit): ?>
                <div class="info-box">📝 Mengatur harga baru akan otomatis menonaktifkan harga lama untuk kategori yang sama.</div>
            <?php endif; ?>

            <div class="card">
                <div class="card-header"><h3>Form Harga</h3></div>
                <div class="card-body">
                    <form method="POST">
                        <div class="form-group">
                            <label>Kategori <span>*</span></label>
                            <select name="kategori_id" class="form-control" required <?= $isEdit ? 'readonly' : '' ?>>
                                <option value="">-- Pilih Kategori --</option>
                                <?php foreach ($kategoriList as $k): ?>
                                    <option value="<?= $k['id'] ?>" <?= (isset($editHarga['kategori_id']) && $editHarga['kategori_id'] == $k['id']) ? 'selected' : '' ?>><?= htmlspecialchars($k['nama']) ?> (<?= $k['jenis'] ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Harga Beli (dari Nasabah) <span>*</span></label>
                            <input type="number" name="harga_beli" class="form-control" value="<?= htmlspecialchars($editHarga['harga_beli'] ?? '') ?>" placeholder="Contoh: 5000" required>
                            <small style="color:#888;">Harga yang dibayar ke warga per kg</small>
                        </div>
                        <div class="form-group">
                            <label>Harga Jual (ke Pengepul) <span>*</span></label>
                            <input type="number" name="harga_jual" class="form-control" value="<?= htmlspecialchars($editHarga['harga_jual'] ?? '') ?>" placeholder="Contoh: 7000" required>
                            <small style="color:#888;">Harga jual ke pengepul per kg</small>
                        </div>
                        <div class="form-group">
                            <label>Berlaku Mulai</label>
                            <input type="date" name="berlaku_mulai" class="form-control" value="<?= $editHarga['berlaku_mulai'] ?? date('Y-m-d') ?>">
                        </div>
                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary">💾 Simpan</button>
                            <a href="<?= base_url('pengelola/harga') ?>" class="btn btn-secondary">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
